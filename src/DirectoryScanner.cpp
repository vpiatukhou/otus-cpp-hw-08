...
void DirectoryScanner::collectFiles(const boost::filesystem::path& rootDirectory, std::size_t currentScanLevel, std::vector<std::unique_ptr<File>>& target) const {
    ...
    for (auto& entry : fs::directory_iterator(rootDirectory)) {
        ...
        } else if (fs::is_regular_file(path) && matchesAnyMask(path.filename().string())) {
...
bool DirectoryScanner::matchesAnyMask(const std::string& filename) const {
    if (fileMasks.empty()) {
        return true;
    }
    for (auto& mask : fileMasks) {
        if (matchesWithMask(filename, mask)) {
            return true;
        }
    }
    return false;
...
void DirectoryScanner::setFileMasks(const std::vector<std::string>& fileMasks) {
